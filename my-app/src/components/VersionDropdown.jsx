  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-medium bg-zinc-800/80 hover:bg-zinc-800 text-zinc-300 hover:text-white border border-zinc-700/60 transition-colors cursor-pointer"
      ...
      {isOpen && (
        <div className="absolute left-0 mt-1.5 w-60 bg-zinc-900 border border-zinc-800 rounded-lg shadow-xl z-[150] overflow-hidden py-1">
          <div className="px-3 py-1.5 text-[10px] font-semibold text-zinc-500 uppercase tracking-wider border-b border-zinc-800/60 flex items-center justify-between">
            <span>Versions</span>
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between px-3 py-2 text-xs cursor-pointer transition-colors group ${
                      isActive ? 'bg-zinc-800/70 text-white' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/40'
                    }`}
                  >
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      <div className="w-4 flex items-center justify-center shrink-0">
                        {isActive && <Check size={13} className="text-indigo-400" />}
                      </div>
          ...
          {!isClient && onOpenAddVersion && (
            <div className="border-t border-zinc-800/80 p-1">
              <button
                ...
                className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 text-xs text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/50 rounded transition-colors"
              >
                <Plus size={13} />
